...
<body class="light-mode">
    ...
    <script>
        ...
        async function uploadToDrive() {
            ...
            if (!file || GOOGLE_SCRIPT_URL.includes("https://script.google.com/macros/s/AKfycbwuh_uFirw6MuAniJEyMTygxegAr9lnt6GNLOeQ7uY-iZgSesN7Be9PDKVxB0I10Zc9CA/exec")) {
                if(GOOGLE_SCRIPT_URL.includes("https://script.google.com/macros/s/AKfycbwuh_uFirw6MuAniJEyMTygxegAr9lnt6GNLOeQ7uY-iZgSesN7Be9PDKVxB0I10Zc9CA/exec")) alert("Please set your Google Script URL in the code!");
                return;
            }
        ...
        function initApp() {
            ...
            if(sessionStorage.getItem('loggedUser') === SUPER_USER && !document.querySelector('.admin-only-item')) {
                const nav = document.getElementById('sidebar-nav');
                const adminBtn = document.createElement('div');
                adminBtn.className = "nav-item admin-only-item";
        ...
        function renderView() {
            ...
            area.innerHTML = filtered.map(m => `
                ...
                </div>`).join('') || '<div style="padding:30px; text-align:center; color:#888;">No mail found.</div>';
        }

        function renderAdminPanel(area) {
            const users = JSON.parse(localStorage.getItem('mailUsers')) || {};
            const emails = JSON.parse(localStorage.getItem('mailDB')) || [];
            area.innerHTML = `
                <div class="admin-section">
                    <h2 style="color:var(--admin-red)">Control Panel</h2>
                    <button class="btn-mid" onclick="localStorage.setItem('mailDB', '[]'); renderView();">Wipe All Mail</button>
                    <p><b>Registered Users</b></p>
                    <table class="admin-table">
                        <tr><th>User</th><th>Pass</th><th>Action</th></tr>
                        ${Object.entries(users).map(([u, p]) => `<tr><td>${u}</td><td><code>${p}</code></td><td><button onclick="wipeUser('${u}')">Del</button></td></tr>`).join('')}
                    </table>
                </div>`;
        }
        ...
        function wipeUser(u) { if(u !== SUPER_USER) { let us = JSON.parse(localStorage.getItem('mailUsers')); delete us[u]; localStorage.setItem('mailUsers', JSON.stringify(us)); renderView(); } }
